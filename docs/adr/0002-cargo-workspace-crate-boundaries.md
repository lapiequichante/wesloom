...
a future feature from accidentally introducing a `wxsl-core -> wxsl-editor`
...
- **`wxsl-core`** — the node/socket/graph data model and the graph → WXSL
  ...
  on.
- **`wxsl-render`** — the `wgpu` renderer: pipeline abstraction and the
  forward/deferred shader-variant switching (ADR 0005). Depends on
  `wxsl-core` only.
- **`wxsl-editor`** — the visual node editor UI (a GUI toolkit
  dependency, see ADR 0004). Depends on `wxsl-core` only. Nothing
  depends on this crate.
- **`wxsl-stdlib`** — the base node library: original shader functions
  (math, color, lighting, SDFs, …) exposed as `wxsl-core` node
  definitions (see ADR 0007). Depends on `wxsl-core` only, ordinary
  workspace MIT/Apache-2.0 license.
- **`wxsl`** — the facade most consumers depend on directly. Re-exports
...
only ever point into `wxsl-core`.** `wxsl-core` and `wxsl-render`
must never depend on `wxsl-editor` or `wxsl-stdlib`, in either the
...
- **Separate repos per crate.** Rejected as premature: nothing here needs
  ...
  cross-crate refactors (which will be common while `wxsl-core`'s data
...
- `wxsl-render` not depending on `wxsl-editor` means the editor is
