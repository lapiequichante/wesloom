...
`wxsl-stdlib`. But the project decided against porting LYGIA at all; see
...
the project wants to offer its functions as ready-to-use `wxsl` base
...
This directly conflicts with the rest of `wxsl` being ordinary
...
- The LYGIA port lives entirely in its own crate, `wxsl-lygia` (ADR
  0002), which carries the Prosperity Public License 3.0.0
  (`crates/wxsl-lygia/LICENSE`, verbatim from upstream) instead of the
  workspace default. `crates/wxsl-lygia/Cargo.toml` sets `license-file`
  instead of inheriting `workspace.package.license`.
- `crates/wxsl-lygia/NOTICE.md` explains the license, attributes LYGIA
  and its author, and keeps a porting log (upstream path → wxsl node →
  upstream commit ported from) so provenance is always traceable per node.
- The crate is `publish = false` and excluded from the `wxsl` facade's
  default features (`lygia` feature, opt-in, ADR 0002's feature table).
  Nobody depends on `wxsl` and gets Prosperity-licensed code without
  explicitly asking for it.
- `AGENTS.md` states the hard rule directly: never move code out of
  `wxsl-lygia` into another crate, and never write a LYGIA "port" that
  isn't genuinely independent and claim it's clean-room.
- Downstream commercial users of `wxsl` have three options once the
...
- **License the whole `wxsl` workspace under Prosperity to match.**
  Rejected: makes the *entire* project noncommercial-by-default, which
  contradicts the goal of `wxsl` being a usable general-purpose
...
- `cargo publish -p wxsl` with the `lygia` feature enabled needs a
...
- Any PR touching `crates/wxsl-lygia/shaders/` must update
