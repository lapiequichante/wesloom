...
ADR 0003 settled that graphs compile to WESL and ADR 0005 that one WESL
...
the hand-written WESL both agree on. Somebody has to own that contract, and
`wesloom-core` is the only crate both halves depend on.
...
switch removes code rather than selecting between two results. WESL has
conditional translation and `const` declarations for exactly this, but a WESL
...
- **A material graph describes a surface.** It compiles to a function
  `wesloom_material(ctx: SurfaceContext) -> Surface`: per-fragment inputs in,
  ...
  G-buffer packing are hand-written WESL.
- **`wesloom_core::abi` is the contract.** Module paths, struct names,
...
- **`wesloom-stdlib` ships the implementation** of that ABI as
  `package::wesloom::{bindings, surface, vertex, shading, deferred,
...
- **Both render paths call the same shading function.** `shade_surface` is
  ...
  the same image.
- **Macro variables** (`wesloom_core::macros`) are the graph-level knobs that
  ...
  - A `Flag` becomes a WESL conditional-translation feature (`@if(name)`).
  - An `Int`/`Float` becomes a `const` declaration in a *generated module*,
    `package::wesloom::macros`, which both the generated material module and
    hand-written stdlib functions import from. That module exists precisely
    because a WESL module cannot reach into the root module importing it.
...
- **The render path is not a macro variable.** `wesloom_deferred` is bound by
...
- **Graphs emit whole shaders (reading 1).** Rejected: it duplicates the
  ...
  avoid.
- **The ABI lives in `wesloom-stdlib`, with core knowing nothing.** Rejected:
  `wesloom-render` needs the entry point and G-buffer names to build
  pipelines, and it must not depend on `wesloom-stdlib` (ADR 0002). Core is
...
- **String-substituting macro values into node WESL before compiling.**
...
- Changing a `SurfaceContext` or `Surface` field is a two-file change:
  `abi.rs`'s table and `shaders/wesloom/surface.wesl`. The tables are
...
- The same applies to the uniform structs: `wesloom-render`'s `scene` module
  mirrors `shaders/wesloom/bindings.wesl` field for field, with a test on the
