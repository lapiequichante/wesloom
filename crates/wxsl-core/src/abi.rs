...
//! [`SURFACE_STRUCT`] of material properties. Everything around that is
//! plumbing with a fixed shape or a generated one: the vertex transform is
//! hand-written WXSL in the modules named here, and the light loop and
//! G-buffer packing are generated from the enabled lighting models
//! ([`crate::lighting`], ADR 0028).
//!
...
/// Module path the *generated* deferred lighting pass is mounted under.
///
/// The pass is no longer a shipped file: `crate::lighting`
/// generates it from the enabled lighting-model set, because a pass that
/// dispatches over models has to name them. The path survives so labels and
/// diagnostics still name something stable.
...
///
/// Generated, since M6, by [`crate::lighting`] rather than hand-written: the
/// light loop is shared, but the per-light model call inside it is the one
/// thing that varies with the enabled lighting models.
pub const SHADE_SURFACE_FN: &str = "shade_surface";
/// The per-light dispatch the generated shading function calls.
///
/// One name, two shapes — both generated by [`crate::lighting`]: a direct
/// call to the material's model in a forward module (the model is known at
/// compile time), and a `switch` over the G-buffer's id in the lighting pass
/// (it is not known until the texel is read).
pub const LIGHTING_DISPATCH_FN: &str = "lighting_dispatch";
/// The G-buffer fragment output struct ([`MaterialStage::GBUFFER`]).
///
/// Generated per lighting-model set: its fields are the set's layout, so it
/// cannot be a fixed shipped struct any more.
...
/// Rebuilds a surface (and the per-model data) from G-buffer texels, in the
/// generated lighting pass.
pub const UNPACK_GBUFFER_FN: &str = "unpack_gbuffer";
...
/// The generated shading function imports it, so both render paths get
/// the same lookup.
...
pub const RESERVED_NAMES: &[&str] = &[
    ...
    "ctx",
    "surface",
    // The two functions the lighting generator pastes into a module that
    // also holds the graph's own: a texture named either would collide with
    // a function, which is a shader-compiler error with no mention of the
    // name the author actually chose.
    SHADE_SURFACE_FN,
    LIGHTING_DISPATCH_FN,
];
...
/// binding, so the shading function needs one lookup rather than one per
/// path.
...
/// occupies.
///
/// The channel count is not cosmetic: a pass's colour attachments share a
/// bytes-per-sample budget ([`MAX_GBUFFER_BYTES_PER_SAMPLE`], in
/// `wxsl-render`), and every vec4 target costs 8 of it regardless of its
/// bit depth — a scalar target costs 1, a two-channel one 4. Targets that
/// only need a value or two declare that here, which is what lets a
/// lighting-model set's requests fit the floor at all.
///
...
pub enum GBufferPrecision {
    ...
    Normalized,
    /// Four values outside `[0, 1]`, or needing a sign: half float.
    HighDynamicRange,
    /// One value in `[0, 1]`, written and read as a bare `f32`.
    NormalizedScalar,
    /// Two half-float values, written and read as a `vec2f`.
    HighDynamicRangePair,
}
...
impl GBufferPrecision {
    ...
    pub fn channels(self) -> usize {
        match self {
            GBufferPrecision::Normalized | GBufferPrecision::HighDynamicRange => 4,
            GBufferPrecision::NormalizedScalar => 1,
            GBufferPrecision::HighDynamicRangePair => 2,
        }
    }

    /// The WGSL type a [`GBUFFER_STRUCT`] field of this precision has.
...
/// The *base* G-buffer layout, in `@location` order.
///
/// Packing two quantities per target keeps the deferred path's fixed cost at
/// three colour attachments plus depth. This is what every deferred pipeline
/// carries; a lighting-model set [`crate::lighting::LightingSet`]
/// *requests* more on top of it — the dispatch id channel and one target per
/// model that needs one — and [`crate::lighting::LightingSet::gbuffer_layout`]
/// is the whole answer. `wxsl-stdlib`'s [`PACK_GBUFFER_FN`] and the lighting
/// pass are the two ends of this contract.
pub const GBUFFER_BASE_TARGETS: &[GBufferTarget] = &[
    GBufferTarget {
        field: "base_color",
...
///
/// A table, like [`GBUFFER_BASE_TARGETS`], and for the same reason: adding a
