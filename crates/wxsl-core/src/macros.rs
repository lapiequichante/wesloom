...
//! A macro is either
//!
//! * a [`MacroValue::Flag`], which becomes a WXSL conditional-translation
//!   feature — `@if(name)` blocks in a node's WXSL are kept or dropped by the
//!   compiler ([ADR 0003](../../../docs/adr/0003-wesl-as-the-shading-language.md)),
//!   or
//! * a [`MacroValue::Int`] / [`MacroValue::Float`], which becomes a
//!   module-scope `const` declaration in the generated WXSL, so a node's
...
//! [`MacroSet::signature`] feeds `wxsl-render`'s variant cache key
...
use crate::wxsl::{write_f32, WxslIdent};

/// The kind of a macro variable, i.e. how it reaches the shader.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum MacroKind {
    /// A boolean, bound as a WXSL conditional-translation feature.
    Flag,
...
pub enum MacroValue {
    /// Bound as a WXSL feature flag: `@if(name)` blocks compile in when true.
    Flag(bool),
...
impl MacroValue {
    ...
    /// This value as a WXSL const declaration, or `None` for a
    /// [`MacroValue::Flag`] (those are bound as compiler features, not
    /// declarations).
    ///
    /// Returns `None` for a non-finite float, which has no WXSL literal.
    pub fn wxsl_const(&self, name: &WxslIdent) -> Option<String> {
        match self {
...
pub struct MacroDef {
    /// The macro name, as written in WXSL (`@if(NAME)` or the const's name).
    pub name: WxslIdent,
...
/// WXSL are byte-for-byte reproducible for the same logical set — the variant
/// cache in `wxsl-render` depends on that.
...
impl MacroSet {
    ...
    /// The `(name, bool)` pairs to bind as WXSL conditional-translation
    ...
    pub fn signature(&self) -> String {
        ...
        for (name, value) in self.iter() {
            ...
            match value {
                ...
                // WXSL literals, so they must not collide here.
...
mod tests {
    ...
    fn flag_has_no_const_declaration() {
        let name = WxslIdent::new("WXSL_X").unwrap();
        assert_eq!(MacroValue::Flag(true).wxsl_const(&name), None);
        assert_eq!(
            MacroValue::Int(4).wxsl_const(&name).unwrap(),
            "const WXSL_X: i32 = 4;"
        );
        assert_eq!(
            MacroValue::Float(2.0).wxsl_const(&name).unwrap(),
            "const WXSL_X: f32 = 2.0;"
        );
        assert_eq!(MacroValue::Float(f32::NAN).wxsl_const(&name), None);
    }
