//! [`ShaderLibrary`]: the WESL modules imports are resolved against.
//!
//! This crate compiles WESL but ships none: the shader ABI and the node
//! library live in `wesloom-stdlib`, which this crate must not depend on
//! ([ADR 0002](../../../docs/adr/0002-cargo-workspace-crate-boundaries.md) —
//! the dependency arrow only points into `wesloom-core`). So the application
//! hands the modules in, and a consumer can substitute its own ABI
//! implementation or add hand-written WESL of its own the same way:
//!
//! ```text
//! let mut library = ShaderLibrary::new();
//! library.insert_all(wesloom_stdlib::MODULES.iter().copied());
//! library.insert("package::game::water", include_str!("water.wesl"));
//! ```
...
/// A set of WESL modules, keyed by module path.
...
impl ShaderLibrary {
    ...
    /// Add many modules, e.g. all of `wesloom_stdlib::MODULES`.
    ...
    /// `wesl` "module not found" on the first material compiled, which is
