...
//! `shaders/wesloom/bindings.wesl`: same fields, same order, same padding.
...
/// and WESL sides disagreeing about the layout. Must match
/// `WESLOOM_MAX_LIGHTS` in `shaders/wesloom/bindings.wesl`.
...
/// Host mirror of `Camera` in `shaders/wesloom/bindings.wesl`.
#[repr(C)]
...
/// Host mirror of `Light` in `shaders/wesloom/bindings.wesl`.
#[repr(C)]
#[derive(Clone, Copy, Debug, Pod, Zeroable)]
...
/// Host mirror of `Scene` in `shaders/wesloom/bindings.wesl`.
#[repr(C)]
#[derive(Clone, Copy, Debug, Pod, Zeroable)]
...
/// Host mirror of `Object` in `shaders/wesloom/bindings.wesl`.
#[repr(C)]
#[derive(Clone, Copy, Debug, Pod, Zeroable)]
...
impl SceneBindings {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let camera = uniform("wesloom camera", size_of::<CameraUniform>() as u64);
        let scene = uniform("wesloom scene", size_of::<SceneUniform>() as u64);
        let object = uniform("wesloom object", size_of::<ObjectUniform>() as u64);
...
mod tests {
    ...
    fn uniform_layouts_match_wgsl_alignment_rules() {
        ...
        assert_eq!(
            ...
            "scene uniform layout drifted from bindings.wesl"
        );
