...
//! the graph, and the WESL it emits.
//!
...
//! WESL. A node in a [`crate::graph::Graph`] is an *instance* of one of these,
//! looked up by id in a [`NodeRegistry`].
//!
//! Two bodies matter most:
//!
//! * [`NodeBody::Expr`] — an inline WESL expression per output, used for the
//!   granular arithmetic that would be silly to wrap in a function
//!   (`{a} + {b}`).
//! * [`NodeBody::Call`] — a call to a [`WeslFunction`], i.e. a real WESL
//!   function shipped as source (`wesloom-stdlib`) or hand-written by the
...
//!   hand-written WESL and from a graph
...
use crate::wesl::{stable_hash, write_f32, ModulePath, WeslIdent};
...
/// no implicit conversion, so a graph that compiles is a graph whose WESL
...
impl ValueType {
    ...
    /// The WGSL/WESL spelling of this type.
    pub fn wesl_type(&self) -> &'static str {
        match self {
            ValueType::Bool => "bool",
            ValueType::I32 => "i32",
            ValueType::U32 => "u32",
...
impl Value {
    ...
    /// This value as a WESL expression, or `None` if any float component is
    /// not finite (WGSL has no `inf`/`nan` literal).
    pub fn wesl_literal(&self) -> Option<String> {
...
impl Socket {
    ...
    /// # Panics
    ///
    /// Panics if `name` is not a valid WESL identifier, or if `default` (when
    ...
    pub fn new(name: &str, ty: ValueType) -> Self {
        Socket {
            name: WeslIdent::new(name).expect("socket name must be a valid WESL identifier"),
            ty,
            default: None,
            optional: false,
            doc: String::new(),
...
/// What a [`WeslFunction`] returns, and therefore what outputs the node has.
#[derive(Clone, Debug, PartialEq)]
pub enum FunctionReturn {
    ...
    Struct {
        ...
        name: WeslIdent,
        /// The fields to expose as outputs. Names must match the WESL struct.
        fields: Vec<Socket>,
    },
}

/// A WESL function usable as a node, described by data.
///
/// This is the bridge between "a shader function someone wrote in WESL" and
/// "a node the graph and editor can reason about": the WESL source stays the
...
impl WeslFunction {
    ...
    pub fn new(module: &str, name: &str, params: Vec<Socket>, ret: Socket) -> Self {
        WeslFunction {
            module: ModulePath::new(module).expect("invalid WESL module path"),
            name: WeslIdent::new(name).expect("invalid WESL function name"),
            params,
    ...
    /// # Panics
    ///
    /// Panics if `module`, `name` or `struct_name` are not valid WESL.
    pub fn new_struct(
        module: &str,
        name: &str,
        params: Vec<Socket>,
        ...
        WeslFunction {
            ...
            ret: FunctionReturn::Struct {
                name: WeslIdent::new(struct_name).expect("invalid WESL struct name"),
                fields,
            },
        }
    }
    ...
    pub fn imports(&self) -> Vec<(ModulePath, WeslIdent)> {
    ...
    /// The function's WESL signature, for docs and error messages.
    pub fn signature(&self) -> String {
        ...
        let ret = match &self.ret {
            FunctionReturn::Value(socket) => socket.ty.wesl_type().to_string(),
...
/// How a node turns its inputs into WESL.
#[derive(Clone, Debug, PartialEq)]
pub enum NodeBody {
    /// One WESL expression per output socket, in output order.
    ///
    /// `{socket}` placeholders are replaced by the expression bound to that
    /// input; `{{` and `}}` are literal braces. Meant for granular
    /// arithmetic — anything with a body belongs in a [`WeslFunction`].
    Expr(Vec<String>),
    /// A call to a WESL function.
    Call(WeslFunction),
...
/// The kind of a node: its typed interface plus the WESL it emits.
#[derive(Clone, Debug, PartialEq)]
pub struct NodeDefinition {
    ...
    /// a [`NodeBody::Call`].
    pub imports: Vec<(ModulePath, WeslIdent)>,
    /// How the node emits WESL.
    pub body: NodeBody,
}
...
impl NodeDefinition {
    ...
    pub fn all_imports(&self) -> Vec<(ModulePath, WeslIdent)> {
...
///
/// Every setter panics on invalid WESL names or mistyped defaults: node
...
impl NodeDefinitionBuilder {
    ...
    /// # Panics
    ///
    /// Panics if `module` or `item` are not valid WESL names.
    pub fn import(mut self, module: &str, item: &str) -> Self {
        self.def.imports.push((
            ModulePath::new(module).expect("invalid WESL module path"),
            WeslIdent::new(item).expect("invalid WESL item name"),
        ));
        self
    }
    ...
    pub fn call(mut self, func: WeslFunction) -> NodeDefinition {
    ...
    /// # Panics
    ///
    /// Panics if `field` is not a valid WESL identifier.
    pub fn context_read(mut self, field: &str) -> NodeDefinition {
        self.def.body =
            NodeBody::ContextRead(WeslIdent::new(field).expect("invalid context field name"));
        self.build()
    }
...
mod tests {
    ...
    fn value_literals_are_wgsl_shaped() {
        assert_eq!(Value::F32(1.0).wesl_literal().unwrap(), "1.0");
        assert_eq!(Value::I32(-2).wesl_literal().unwrap(), "-2i");
        assert_eq!(Value::U32(7).wesl_literal().unwrap(), "7u");
        assert_eq!(Value::Bool(true).wesl_literal().unwrap(), "true");
        assert_eq!(
            Value::Vec3([0.5, 1.0, 0.0]).wesl_literal().unwrap(),
            "vec3f(0.5, 1.0, 0.0)"
        );
        assert!(Value::Vec2([1.0, f32::NAN]).wesl_literal().is_none());
    }
    ...
    fn function_nodes_take_their_interface_from_the_function() {
        let func = WeslFunction::new_struct(
