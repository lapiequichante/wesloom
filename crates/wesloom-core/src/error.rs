...
//! `wesloom-core` dependency-light" rule in `AGENTS.md`.
...
pub enum GraphError {
    ...
    /// A macro name pinned by the graph is not a valid WESL identifier, so it
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GraphError::InvalidMacroName { name } => {
                write!(f, "macro name `{name}` is not a valid WESL identifier")
            }
...
/// A problem that stopped [`crate::codegen::generate`] from emitting WESL.
#[derive(Clone, Debug, PartialEq)]
#[non_exhaustive]
pub enum CodegenError {
    /// The graph did not validate. Codegen always validates first: emitting
    /// WESL from an invalid graph would just move the error into the shader
    ...
    /// A value could not be written as a WESL literal (non-finite float).
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CodegenError::UnrepresentableValue { socket } => {
                write!(f, "value for {socket} cannot be written as a WESL literal")
            }
        }
    }
}
