//! Turns a validated graph into WESL source, ready for the `wesl` compiler to
...
//!    that is the whole reason graphs compile to WESL rather than to WGSL
//!    ([ADR 0003](../../../docs/adr/0003-wesl-as-the-shading-language.md)).
//! 2. **Macro imports** — the numeric macro variables in effect, imported
//!    from the module [`macro_module`] generates. Flag macros are not
//!    imported: they are bound as WESL conditional-translation features by
...
use crate::wesl::{stable_hash, ModulePath, WeslIdent};

/// Module path the generated material module is mounted at.
///
/// `wesloom-render` adds the generated source to its resolver under this
...
pub struct CodegenOptions {
    ...
    /// importing a graph's material into hand-written WESL.
...
/// WESL source generated from a graph, plus everything needed to compile it.
#[derive(Clone, Debug, PartialEq)]
pub struct GeneratedShader {
    /// The WESL source of the root module.
    pub source: String,
...
impl GeneratedShader {
    ...
    ///
    /// The source alone does not identify the compiled shader: flag macros
    /// are bound as compiler features, and numeric ones are declared in the
    /// generated macro module ([`macro_module`]), so neither shows up in the
    /// root module. `wesloom-render` combines this with the render path to key
    /// its variant cache.
...
/// Generate WESL for `graph`.
///
/// The graph is validated first: emitting WESL from an invalid graph would
...
pub fn generate(
    ...
    // Numeric macros are declared in the generated macro module, so a node
    // expression that reads one needs it imported here too.
    for (name, value) in macros.iter() {
        if !matches!(value, MacroValue::Flag(_)) {
            emitter.request_import(abi::MACROS_MODULE, name);
        }
    }
...
struct Emitter<'a> {
    ...
    imports: BTreeMap<ModulePath, Vec<WeslIdent>>,
...
impl Emitter<'_> {
    fn request_import(&mut self, module: &str, item: &str) {
        ...
        let item = WeslIdent::new(item).expect("ABI item names are valid");
    ...
    /// The WESL expression feeding `node`'s input `socket`, or `None` if the
    ...
    fn finish(self, graph: &Graph, macros: &MacroSet, surface: &SurfaceAssignments) -> String {
        ...
        let _ = writeln!(
            out,
            "// Generated by wesloom-core from graph `{}`.",
            graph.name()
        );
        ...
        for (module, items) in &imports {
            let mut items: Vec<&str> = items.iter().map(WeslIdent::as_str).collect();
            items.sort_unstable();
...
const MACRO_MODULE_HEADER: &str = "\
// Generated by wesloom-core: the macro variables this shader was compiled
// with. Import the ones you need, e.g.
// `import package::wesloom::macros::WESLOOM_FBM_OCTAVES;`.
";

/// The WESL source of the macro module for `macros`, to be mounted at
/// [`abi::MACROS_MODULE`] next to the generated material module.
///
/// Every numeric macro becomes one `const` declaration; flag macros are
/// absent, because `@if` needs a compiler feature rather than a declaration.
///
/// This is a separate module rather than declarations in the generated one
/// because a WESL module cannot see into the root module that imports it — so
/// a shared module is the only place a host-set constant can live where both
/// the generated material and a hand-written stdlib function can read it.
pub fn macro_module(macros: &MacroSet) -> String {
    let mut out = String::from(MACRO_MODULE_HEADER);
    for (name, value) in macros.iter() {
        let Some(ident) = WeslIdent::new(name) else {
            continue;
        };
        if let Some(declaration) = value.wesl_const(&ident) {
...
mod tests {
    ...
    use crate::node::{Socket, ValueType, WeslFunction};
    ...
    fn registry() -> NodeRegistry {
        ...
        registry.register_all([
            ...
            NodeDefinition::builder("test.macro_user", "Macro user")
                ...
                .input(Socket::new("a", ValueType::F32).with_splat_default(1.0))
                .output(Socket::new("out", ValueType::F32))
                .expr("{a} * WESLOOM_TEST_SCALE"),
            NodeDefinition::builder("test.split", "Split").call(WeslFunction::new_struct(
                "package::test",
                "split_value",
    ...
    fn a_bare_output_node_still_compiles_to_a_material() {
        ...
        assert!(shader
            .source
            .contains("fn wesloom_material(ctx: SurfaceContext) -> Surface"));
        assert!(shader
            .source
        ...
        assert!(shader.source.contains("@if(!wesloom_deferred)"));
        assert!(shader.source.contains("@if(wesloom_deferred)"));
    ...
    fn macros_reach_the_shader_as_consts_and_features() {
        ...
        let shader = generate_default(&graph, &registry);
        assert_eq!(
            shader.macros.get("WESLOOM_TEST_SCALE"),
            Some(MacroValue::Float(2.0))
        );
        assert_eq!(
            shader.macros.get("wesloom_test_flag"),
            Some(MacroValue::Flag(false))
        );
        // The numeric macro is imported from the macro module; the flag is
        // not, because a flag is a compiler feature, not a declaration.
        assert!(shader
            .source
            .contains("import package::wesloom::macros::WESLOOM_TEST_SCALE;"));
        assert!(!shader.source.contains("wesloom_test_flag;"));
        assert!(macro_module(&shader.macros).contains("const WESLOOM_TEST_SCALE: f32 = 2.0;"));
        assert!(!macro_module(&shader.macros).contains("wesloom_test_flag"));

        graph.set_macro("WESLOOM_TEST_SCALE", MacroValue::Float(8.0));
        graph.set_macro("wesloom_test_flag", MacroValue::Flag(true));
        let pinned = generate_default(&graph, &registry);
        assert!(macro_module(&pinned.macros).contains("const WESLOOM_TEST_SCALE: f32 = 8.0;"));
        assert_eq!(
            pinned.macros.get("wesloom_test_flag"),
            Some(MacroValue::Flag(true))
        );

        // Neither kind of macro changes the root module beyond the header
        // comment recording what it was built with, which is why the variant
        // key folds in the whole macro set instead of trusting the source.
    ...
    fn entry_points_can_be_left_out() {
        ...
        assert!(shader.source.contains("fn wesloom_material"));
    }
}
