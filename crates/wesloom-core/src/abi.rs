//! The shader ABI: the fixed WESL vocabulary a generated material module is
//! written against.
//!
...
//! transform, light loop, G-buffer packing) is hand-written WESL living in the
//! modules named here, which `wesloom-stdlib` ships.
//!
//! This module is the single place where the two halves agree on names, so
//! drift shows up as one edit here plus one in the `.wesl` source rather than
...
/// [`DEFAULT_SURFACE_FN`].
pub const SURFACE_MODULE: &str = "package::wesloom::surface";
/// Module holding the vertex stage and the context builder.
pub const VERTEX_MODULE: &str = "package::wesloom::vertex";
/// Module holding the forward-path shading function.
pub const SHADING_MODULE: &str = "package::wesloom::shading";
/// Module holding the G-buffer struct and packing function.
pub const DEFERRED_MODULE: &str = "package::wesloom::deferred";
/// Module holding the numeric macro variables in effect, generated per
/// compilation by [`crate::codegen::macro_module`].
///
/// Both the generated material module and hand-written stdlib functions
/// import their macros from here: a WESL module cannot see declarations in
/// the root module that imports it, so a shared virtual module is the only
/// place a host-set constant can live.
pub const MACROS_MODULE: &str = "package::wesloom::macros";
...
/// Vertex stage input struct (matches `wesloom-render`'s vertex buffer layout).
...
pub const LIGHTING_PASS_MODULE: &str = "package::wesloom::lighting_pass";
...
/// needs; `wesloom-render` maps that onto concrete `wgpu` texture formats,
...
/// attachments plus depth. `wesloom-stdlib`'s [`PACK_GBUFFER_FN`] and the
...
pub const FEATURE_TONEMAP: &str = "wesloom_tonemap";
/// Feature flag: output normals instead of shading (see [`abi_macros`]).
pub const FEATURE_DEBUG_NORMALS: &str = "wesloom_debug_normals";

/// The WESL conditional-translation feature that selects the deferred path.
///
/// Bound by `wesloom-render` from the pipeline's `RenderPath`, never stored on
...
pub const FEATURE_DEFERRED: &str = "wesloom_deferred";

/// Name of the generated material function.
pub const MATERIAL_FN: &str = "wesloom_material";
...
pub struct ContextField {
    /// Field name in the WESL struct, and the node id suffix.
    pub name: &'static str,
    /// Field type.
    pub ty: ValueType,
    /// Editor label.
...
///
/// The `.wesl` struct must list exactly these, in this order.
...
pub struct SurfaceField {
    /// Field name in the WESL struct, and the input socket name.
    pub name: &'static str,
    /// Field type.
    pub ty: ValueType,
