...
//! `assets/pbr_cube.wesloom.json`, the node format, and compiled to WESL at
...
//! and the difference is conditional translation inside one WESL module
//! (ADR 0005).
//!
//! ```text
...
//! cargo run --example pbr_cube -- --dump-wesl        # what the graph became
//! cargo run --example pbr_cube -- --dump-wgsl --path deferred
//! cargo run --example pbr_cube -- --list-macros
//! cargo run --example pbr_cube -- --macro WESLOOM_FBM_OCTAVES=2
//! ```
//!
...
const DEFAULT_GRAPH: &str = include_str!("../assets/pbr_cube.wesloom.json");

const USAGE: &str = "\
pbr_cube — a PBR cube whose material is a wesloom node graph
...
OPTIONS:
    ...
    --dump-wesl                Print the WESL generated from the graph and exit
    ...
    fn parse(args: impl Iterator<Item = String>) -> Result<Self, String> {
        ...
        while let Some(arg) = args.next() {
            ...
            match arg.as_str() {
                ...
                "--dump-wesl" => options.dump_wesl = true,
...
impl App {
    ...
    fn on_key(&mut self, code: KeyCode, event_loop: &ActiveEventLoop) {
        match code {
            ...
            KeyCode::KeyR => self.toggle_flag("wesloom_fbm_ridged"),
